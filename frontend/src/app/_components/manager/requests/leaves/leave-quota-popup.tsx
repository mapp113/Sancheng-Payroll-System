...
export default function LeaveQuotaPopup({ onClose }: LeaveQuotaPopupProps) {
  const [employeeCode, setEmployeeCode] = useState("");
  const [year, setYear] = useState(new Date().getFullYear().toString());
  const [data, setData] = useState<LeaveQuotaYearSummaryResponse | null>(null);
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/config/summary?employeeCode=${encodeURIComponent(employeeCode)}&year=${year}`,
        {
          headers: {
    ...
    <FormPopBoxNotScroll>
      <div className="w-[600px] max-w-full">
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-bold text-[#1D3E6A]">
            Tra cứu số ngày nghỉ phép
          </h2>
          <button
            onClick={onClose}
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-[#b8e9f7]">
                    <th className="px-3 py-2 text-left text-xs font-semibold text-[#1D3E6A] border border-gray-300">
                      Loại nghỉ
                    </th>
                    <th className="px-3 py-2 text-right text-xs font-semibold text-[#1D3E6A] border border-gray-300">
                      Số ngày được nghỉ
                    </th>
                    <th className="px-3 py-2 text-right text-xs font-semibold text-[#1D3E6A] border border-gray-300">
                      Chuyển từ năm trước
                ...
                <tbody>
                  {data.quotas.map((quota, index) => (
                    <tr
                      key={quota.leaveTypeCode}
                      className={index % 2 === 0 ? "bg-white" : "bg-[#f8fcff]"}
                    >
                      <td className="px-3 py-2 text-sm text-[#1D3E6A] border border-gray-300">
                        {quota.leaveTypeName}
                      </td>
                      <td className="px-3 py-2 text-sm text-[#1D3E6A] text-right border border-gray-300">
                        {quota.entitledDays}
                      </td>
                      <td className="px-3 py-2 text-sm text-[#1D3E6A] text-right border border-gray-300">
                        {quota.carriedOver}
                      ...
                      <td className="px-3 py-2 text-sm font-semibold text-[#1D3E6A] text-right border border-gray-300">
                        {quota.remainingDays}
                      </td>
                    </tr>
                  ))}
